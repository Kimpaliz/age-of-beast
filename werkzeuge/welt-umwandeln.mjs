...
import { mitRahmen } from './welt-rahmen.mjs';
...
export function umwandeln(roh) {
  ...
  const rohElemente = mitRahmen(roh, alsAbsaetze);
  ...
  for (const kategorieInfo of KATEGORIEN) {
    ...
    for (const [id, element] of Object.entries(gruppe)) {
      ...
      eintraege.push({
        ...
        // Die Spielwerte einer Figur werden **als Struktur** durchgereicht,
        // nicht als Text: Der Charakterbogen zeichnet daraus Leisten,
        // Attributfelder und Schadensschwellen. Waeren sie Fliesstext in
        // einem Abschnitt, muesste der Bogen sie wieder auseinandernehmen —
